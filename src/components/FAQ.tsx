      ...
      <div className="max-w-4xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
        <div className="text-center mb-8 sm:mb-10 md:mb-12 sm:mb-16">
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 mb-4 sm:mb-5 md:mb-6">
            <div className="p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-accent/10">
              <HelpCircle className="w-6 h-6 sm:w-7 sm:h-7 lg:w-8 lg:h-8 text-accent" />
            </div>
            <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-primary">
              Frequently Asked Questions
            </h2>
          </div>
          <p className="text-sm sm:text-base md:text-lg lg:text-xl xl:text-2xl text-muted-foreground px-2 font-medium leading-relaxed">
        ...
        <Accordion type="single" collapsible className="w-full space-y-3 sm:space-y-4">
          {faqs.map((faq, index) => (
            ...
            >
              <AccordionTrigger className="text-left text-sm sm:text-base lg:text-lg hover:no-underline font-semibold py-4 sm:py-5 lg:py-6 text-primary">
                <div className="flex items-start gap-3 sm:gap-4 pr-3 sm:pr-4">
                  <MessageCircleIcon className="w-4 h-4 sm:w-5 sm:h-5 flex-shrink-0 mt-0.5 sm:mt-1 text-accent" />
                  <span className="leading-tight">{faq.question}</span>
                </div>
              </AccordionTrigger>
              <AccordionContent className="text-muted-foreground text-sm sm:text-base leading-relaxed pb-4 sm:pb-5 lg:pb-6 pl-7 sm:pl-9 font-medium">
                {faq.answer}
              </AccordionContent>
            </AccordionItem>
          ))}
