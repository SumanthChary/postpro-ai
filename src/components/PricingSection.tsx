...
const PricingSection = () => {
  ...
  const oneTimePlans = lifetimePlansRaw.filter(plan => plan.name.toLowerCase().includes("one-time") || plan.badge?.toLowerCase().includes("one-time"));
  const lifetimePlans = lifetimePlansRaw.filter(plan => !oneTimePlans.some(oneTimePlan => oneTimePlan.name === plan.name));
  ...
  return <section className="py-24 bg-gradient-to-br from-blue-50/50 via-white to-purple-50/30">
      <div className="container mx-auto px-4">
        ...
        {(lifetimePlan || oneTimePlans.length > 0) && <div className="mt-20 max-w-5xl mx-auto px-4">
            <div className={`grid gap-8 ${lifetimePlan && oneTimePlans.length > 0 ? "lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)]" : "lg:grid-cols-1"}`}>
              {lifetimePlan && <div className="relative overflow-hidden rounded-[32px] border-[3px] border-red-200 bg-white shadow-xl">
                  ...
                  <div className="p-8 sm:p-10 md:p-12">
                    <div className="flex flex-col gap-6">
                      ...
                      <ul className="space-y-3">
                        {lifetimePlan.features.slice(0, 3).map(feature => <li key={feature} className="flex items-start gap-3">
                      ...
                      <Button className="mt-2 w-full rounded-xl bg-red-600 py-4 text-base font-semibold text-white shadow-sm transition-colors duration-200 hover:bg-red-700" onClick={() => handleSubscribe(lifetimePlan)}>
                        {lifetimePlan.cta}
                      </Button>
                    </div>
                  </div>
              ...
              {simplifiedOneTime && <div className="flex flex-col gap-5 rounded-[32px] border-2 border-blue-100 bg-white/95 p-6 sm:p-8 shadow-lg">
                  ...
                  <div className="space-y-3">
                    {simplifiedOneTime.features.map(feature => <div key={feature} className="flex items-start gap-3">
                        <CheckCircle className="mt-0.5 h-4 w-4 flex-shrink-0 text-blue-600" />
                        <span className="text-sm text-gray-700">{feature}</span>
                      </div>)}
                  </div>

                  <Button className="mt-2 w-full rounded-xl bg-blue-600 py-3 text-sm font-semibold text-white shadow-sm transition-colors duration-200 hover:bg-blue-700" onClick={() => handleSubscribe(simplifiedOneTime)}>
